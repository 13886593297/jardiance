...
export default {
    ...
    beforeDestroy() {
        ...
        document.querySelectorAll('meta')[2].setAttribute('content', 'width=device-width,initial-scale=1.0,maximum-scale=1.0')
    },
    methods: {
        back() {
        ...
        urlHandler(src) {
            if (src.endsWith('pdf')) {
                this.urlType = 'pdf'
                this.url = pdf.createLoadingTask({ url: src, CMapReaderFactory })
                document.querySelectorAll('meta')[2].setAttribute('content', 'width=device-width,user-scalable=yes')
